<x-app-layout>
    <section class="w-full flex bg-fixed" style="background-image: url({{ asset('img/home/portada01.png') }})">
        <div class="max-w-max mx-auto px-4 sm:px-6 lg:px-8 py-5">
            <!-- component -->
            <div class="py-6">
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg p-4">
                        <!-- Encabezado agregar y buscar -->
                        <div class="sm:flex-1 sm:flex sm:items-center sm:justify work-sans">
                            <!-- BOTON AGREGAR -->
                            <!-- component -->
                            <div class="p-3">
                                <button onclick="openModal(true)"
                                    class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded my-3 mx-2">
                                    Agregar Muestra
                                </button>
                            </div>
                        ...
                        <table class="min-w-full max-w-max w-full table-auto">
                            <!--min-w-full divide-y divide-gray-200 mt-4-->
                            <!--min-w-max max-w-max w-full table-auto-->
                            <!-- ENCABEZADOS -->
                            <thead>
                                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                    <th class="py-3 px-6 text-left">ID</th>
                                    <th class="py-3 px-6 text-left">PLANTA</th>
                                    <th class="py-3 px-6 text-center">UBICACION</th>
                                    <th class="py-3 px-6 text-center">ESTADO</th>
                                    <th class="py-3 px-6 text-center">RESPONSABLE</th>
                                    <th class="py-3 px-6 text-center">FECHA</th>
                                    <th class="py-3 px-6 text-center">ELEMENTOS</th>
                                    ...
                                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        ...
                                        <td class="py-3 px-6 text-left whitespace-nowrap">
                                            <div class="flex items-center">
                                                <span class="font-medium">{{ $muestra['id'] }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-3 px-6 text-left">
                                            <div class="flex items-center">
                                                <span>{{ Str::limit($muestra->planta->nombre_comun, 30) }}</span>
                                            </div>
                                        </td>
                                            ...
                                            <div class="flex items-center">
                                                <div class="mr-2">
                                                    <img class="h-8 w-8 rounded-full object-cover"
                                                        src="{{ $muestra->user->profile_photo_url }}"
                                                        alt="{{ $muestra->user->name }}" />
                                                </div>
                                                <span>{{ Str::limit($muestra->user->name, 30) }}</span>
                                            </div>
                                        </td>
                                        {{-- @foreach ($users as $user)

                                            @if ($user['id'] == $muestra['user_id'])
                                                <td class="py-3 px-6 text-left">
                                                    <div class="flex items-center">
                                                        @auth
                                        ...
                                        <td class="py-3 px-6 text-left whitespace-nowrap">
                                            <div class="flex item-center justify-center">
                                                <a href="{{ route('elementos.index')}}"
                                                    class="bg-yellow-500 hover:bg-yellow-700 text-md text-white font-bold py-1 px-3 rounded mx-1">
                                                    Elementos
                                                </a>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-3 px-6 text-left whitespace-nowrap">
                                            <div class="flex item-center justify-center">
                                                <button onclick="mostar({{ $muestra->id }})"
                                                    class="bg-blue-500 hover:bg-blue-700 text-sm text-white font-bold py-1 px-3 rounded mx-1">
                                                    Editar
                                                </button>
                                                <button
                                                    class="bg-red-500 hover:bg-red-700 text-sm text-white font-bold py-1 px-3 rounded mx-1">
                                                    Borrar
                                                </button>
                                            </div>
                                        </td>
                        ...
                        <div class="sm:flex-1 sm:flex sm:items-center sm:justify-between mt-4 work-sans">
                            ...
                            <div>
                                <nav class="relative z-0 inline-flex shadow-sm">
                                    ...
                                    <div>
                                        <a href="#"
                                            class="-ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-bold text-black-700 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-tertiary active:text-gray-700 transition ease-in-out duration-150 hover:bg-tertiary">
                                            1
                                        </a>
                                        <a href="#"
                                            class="-ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-bold text-black-600 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-tertiary active:text-gray-700 transition ease-in-out duration-150 hover:bg-tertiary">
                                            2
                                        </a>
                                        <a href="#"
                                            class="-ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-bold text-black-600 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-tertiary active:text-gray-700 transition ease-in-out duration-150 hover:bg-tertiary">
                                    ...
                                    <div v-if="pagination.current_page < pagination.last_page">
                                        <a href="#"
                                            class="-ml-px relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm leading-5 font-bold text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
                                            aria-label="Next">
                                            <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                <path fill-rule="evenodd"
    ...
    <div id="modal_overlay" class="fixed z-10 inset-0 overflow-y-auto ease-out duration-400">

        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">

            <!-------color y opacidad modal------------>
            <div id="modal_overlay2" class="fixed inset-0 transition-opacity">
                <div id="modal_overlay3" class="absolute inset-0 bg-black opacity-50"></div>
            </div>
            ...
            <div id="modal" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
                ...
                <div class="flex flex-row justify-between p-3 bg-green-100 border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
                    <p class="font-semibold text-green-600 font-bold text-xl-center">  AGREGAR MUESTRA  </p>
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 30 24" onclick="openModal(false)"
                        xmlns="http://www.w3.org/2000/svg">
                ...
                <form method="POST" action=" {{ route('muestras.save') }}">
                    @csrf
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="">
                            <div class="mb-4">
                                ...
                                <select class="w-full border bg-white rounded px-3 py-2 outline-none" name="planta_id">
                                    <option class="py-1" value="" selected disabled>Desplegar plantas</option>
                                    @foreach ($plantas as $planta)
                                        <option class="py-1" value="{{$planta['id']}}">{{$planta['nombre_comun']}}</option>
                                    @endforeach
                                </select>
                                @error('planta_id')
                            ...
                            <div class="mb-4">
                                <label for="nombreInput" class="block text-gray-700 text-sm font-bold mb-2">
                                    Ubicacion:
                                </label>
                                <input type="text"
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    id="nombreInput" placeholder="Nombre del lugar" name="ubicacion" />
                                @error('ubicacion')
                                <div class="text-red-500">
                                        El campo ubicación es requerido*
                                </div>
                                @enderror
                            </div>
                            <div class="mb-4">
                                <label for="descripcionInput" class="block text-gray-700 text-sm font-bold mb-2">
                                    Fecha:
                                </label>
                                <input type="date" name="fecha" required value="{{date('Y-m-d')}}"  class="border border-gray-300 p-2 my-2 rounded-md focus:outline-none focus:ring-2 ring-blue-200" />
                            ...
                            <div class="mb-4">
                                ...
                                <select class="w-full border bg-white rounded px-3 py-2 outline-none"
                                    name="user_id">
                                    <option class="py-1" value="" selected disabled>Desplegar usuarios</option>
                                    @foreach ($users as $user)
                                        <option class="py-1" value="{{$user['id']}}">{{$user['name']}}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                <div class="text-red-500">
                    ...
                    <div class="bg-gray-100 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                            <button  type="submit"
                                class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-green-600 text-base leading-6 text-white font-bold shadow-sm hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                                Guardar
                            </button>
                        </span>
                        <!--<span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                            <button wire:click.prevent="store()" type="button"
                                class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-green-600 text-base leading-6 text-white font-bold shadow-sm hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green transition ease-in-out duration-150 sm:text-sm sm:leading-5"
                                v-show="editMode" onclick="openModal(false)">
                                Actualizar
                            </button>
                        </span>-->
                        <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
                            <button onclick="openModal(false)" type="button"
                                class="inline-flex justify-center w-full rounded-md border border-gray-300 px-4 py-2 bg-red-500 leading-6 text-white font-bold shadow-sm hover:bg-red-700 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                                Cancelar
                            </button>
                        </span>
                    </div>
                    <!--<footer class="flex justify-center bg-transparent">
                        <button class="bg-green-600 font-semibold text-white py-3 w-full rounded-b-md hover:bg-green-700 focus:outline-none focus:ring shadow-lg hover:shadow-none transition-all duration-300">
                          Confirm
                        </button>
                      </footer>-->
                </form>
...
<script>
    ...
    function openModal(value) {
        ...
        const overlay2Cl = modal_overlay2
        const overlay3Cl = modal_overlay3

        if (value) {
            setTimeout(() => {
                modalCl.add('inline-block')
                modalCl.add('align-bottom')
                modalCl.add('bg-white')
                modalCl.add('text-left')
                modalCl.add('overflow-hidden')
                modalCl.add('shadow-xl')
                modalCl.add('transform')
                modalCl.add('transition-all')
                modalCl.add('sm:my-8')
                modalCl.add('sm:align-middle')
                modalCl.add('sm:max-w-lg')
                modalCl.add('sm:w-full')
            });
            setTimeout(() => overlayCl.classList.add('inset-0'));
            setTimeout(() => overlay2Cl.classList.add('inset-0'));
            setTimeout(() => overlay3Cl.classList.add('inset-0'));
        } else {
            overlayCl.classList.remove('inset-0')
            overlay2Cl.classList.remove('inset-0')
            overlay3Cl.classList.remove('inset-0')
            setTimeout(() => {
                modalCl.remove('inline-block')
                modalCl.remove('align-bottom')
                modalCl.remove('bg-white')
                modalCl.remove('text-left')
                modalCl.remove('overflow-hidden')
                modalCl.remove('shadow-xl')
                modalCl.remove('transform')
                modalCl.remove('transition-all')
                modalCl.remove('sm:my-8')
                modalCl.remove('sm:align-middle')
                modalCl.remove('sm:max-w-lg')
                modalCl.remove('sm:w-full')
            });
        }
    }
    /*@if ($errors->any())
